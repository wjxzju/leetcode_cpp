#ifndef __LEETCODE_39
#define __LEETCODE_39
#include <iostream>
#include <vector>
#include <algorithm>
using namespace std;
...
public:
    ...
    static void process(int index,const vector<int>& candidates, int target, vector<vector<int>>& res,vector<int> & tmp)
	{
		...
		for (int i = index; i < candidates.size() && target>=candidates[i]; i++){
			if ((i < candidates.size() - 1 && candidates[i] != candidates[i + 1])  || (i== candidates.size()-1) )
			{
				tmp.push_back(candidates[i]);
				process(i,candidates, target - candidates[i], res, tmp);
				tmp.pop_back();
			}
